...
export default class Glyph extends Randomizable {
  ...
  constructor(
    ...
    if (this.leet) {
      this.glyphs.push(...this.getLeetValues(lower));
    }
    if (this.unicode) {
      this.glyphs.push(...this.getUnicodeValues(lower));
    }
  }

  private getLeetValues(character: string): Array<string> {
    switch (character) {
      case 'a':
        return leetJSON.a;
      case 'b':
  ...
  private getUnicodeValues(character: string): Array<string> {
