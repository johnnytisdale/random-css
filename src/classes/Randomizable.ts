export default abstract class Randomizable {
  ...
  public static getRandomArrayElement<T>(array: Array<T>) {
    return array[Randomizable.getRandomNumber(0, array.length - 1)];
  }

  public static getRandomBoolean(): boolean {
    return Randomizable.getRandomNumber(0, 1) === 1;
  }

  public static getRandomDecimal(min = 0, max = 1, places = 2): number {
    if (min < 0) {
  ...
  public static getRandomNumber(min: number, max: number, integerOnly = true) {
